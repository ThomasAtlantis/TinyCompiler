...
int main() {
    ...
    grammar.add("A2") >> "A3 A2" | "null";
    grammar.add("A3") >> "A4" | "A5" | "A6";
    grammar.add("A4") >> "A7 ;";
    grammar.add("A6") >> "if ( A8 ) { A2 } A9";
    grammar.add("A9") >> "else B0" | "null";
    grammar.add("B0") >> "{ A2 }" | "A6";
    grammar.add("A5") >> "while ( A8 ) { A2 B1 A2 }";
    grammar.add("A5") >> "for ( B2 ; B3 ; B4 ) { A2 B1 A2 }";
    grammar.add("B2") >> "A7" | "null";
    grammar.add("B3") >> "A8" | "null";
    grammar.add("B4") >> "A7" | "null";
    grammar.add("B1") >> "continue ;" | "break ;";
    grammar.add("B5") >> "putc B6" | "puts B7";
    grammar.add("B6") >> "B8" | "B9";
    grammar.add("C0") >> "= A8" | "null";
    grammar.add("C1") >> "C2 = A8";
    ...
    grammar.add("D7") >> "B8 C6" | "C5 quap" | "( A8 )";
    grammar.add("A7") >> "D8" | "C1" | "return" | "B5" | "struct B8 D9";
