...
int main() {
    ...
    grammar.add("B7") >> "E8" | "A4 quap qua_check_def_e C8" | "return" | "C4";
    ...
    grammar.add("F2") >> "A4 qua_declare_id quap_id F3";
    grammar.add("F3") >> "B1 qua_declare" | "F4 F5";
    grammar.add("B1") >> "[ F6 qua_declare_arr ]" | "null";
    grammar.add("F5") >> "= B8 qua=";
    grammar.add("F4") >> "null qua_declare";
    grammar.add("F0") >> "A9 A4 B1" | "struct A4 A4";
    grammar.add("F1") >> "F0 ; F1" | "null";
    grammar.add("A4") >> "@I";
    grammar.add("F7") >> "A4 quap qua_check_def_e" | "F6 quap";
    grammar.add("D1") >> "[ F7 ] qua[]" | "null";
    grammar.add("D5") >> "F6" | "F8" | "F9";
    grammar.add("F6") >> "@INT";
    grammar.add("F8") >> "@FLT";
    grammar.add("C7") >> "@CH";
    grammar.add("C6") >> "@STR";
    grammar.add("F9") >> "true" | "false";
